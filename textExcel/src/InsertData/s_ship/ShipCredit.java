    ...
    public static void shipCredit() {
        ...
        for (int i=0;i<=2500;i++) {
            ...
            try {
                ...
                j = random.nextInt(shipCreditList.size());
                pstm.setString(4,shipCreditList.get(j).score);
                pstm.setObject(5,randomDate.generateRandomDate("2020-01-01","2020-10-13"));
                pstm.setString(6,violationLoc[random.nextInt(violationLoc.length)]);
                pstm.setString(7,shipCreditList.get(j).describe);
