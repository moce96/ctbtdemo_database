...
public class GetPersonId {
    ...
    public List<Person> getAllByExcel() {

        List<Person> list=new ArrayList<>();
        try {
            Workbook rwb=Workbook.getWorkbook(new File("src\\excel\\person.xls"));
            ...
            for (int i = 1; i < rows; i++) {
                for (int j = 0; j < clos; j++) {
                    ...
                    Date endTime =  StringToDate.function(rs.getCell(j++, i).getContents());

//                    System.out.println(name+id);

                    list.add(new Person(name,id,bloodType,shipId,startTime,endTime));
                }
            }
        } catch (Exception e) {
